...
const Documents = () => {
  ...
  const { list, loading, error, count, searchResults, createStatus } = useSelector(state => state.documents)
  ...
  // Filter by category
  const filteredDocuments = documentsToDisplay.filter(
    (doc) => {
  ...
  const availableCategories = [...new Set(list.map(doc => doc.category || 'Other'))]
  ...
  const DocumentCard = ({ doc, index }) => {
    const fileType = doc.url ? getFileTypeFromUrl(doc.url) : getFileType(doc.name)
      ...
      >
        ...
        <div className="relative bg-gradient-to-r from-gray-50 to-gray-100 p-4 border-b border-gray-100">
          <div className="flex items-center justify-between">
            <motion.div 
              className={`px-3 py-1 rounded-full text-xs font-bold ${getTypeColor(fileType)} shadow-md flex items-center space-x-1`}
        ...
        <div className="p-6">
          ...
          <div className="flex items-start space-x-4 mb-4">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-bold text-lg text-gray-900 truncate mb-1" title={doc.name}>
                {doc.name}
              </h3>
          ...
          <div className="space-y-3 mb-6">
            ...
            <div className="flex items-center space-x-2 text-sm text-gray-600">
              ...
              <span className="text-gray-800">{doc.category || 'Uncategorized'}</span>
            </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div>
            ...
            >
              Manage and organize all your files {username && `- Welcome ${username}`}
            </motion.p>
          </div>
          <motion.button
            ...
            >
              ...
              {availableCategories.map(category => (
                <option key={category} value={category}>
                  {category} ({list.filter(doc => (doc.category || 'Other') === category).length})
                </option>
              ))}
            </motion.select>
          </div>
          ...
          {!loading && filteredDocuments.length === 0 && (
            ...
            >
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">
                {searchTerm.trim() ? `No documents found for "${searchTerm}"` : 
                 selectedCategory !== "all" ? `No documents in "${selectedCategory}" category` : 
                 "No documents found"}
              </h3>
